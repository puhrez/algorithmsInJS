function sort() {
  ...
  var publicAPI = {
    ...
    selectionSort: function selectSort(arr) {
      /*
      * selectonSort works by iterating through the array to find the smallest
      * element and then setting that to the position of the current index.
       * best case: already sorted, theta(n - 1)
      * worst case: reverse sorted, n(n+1)/2 ~ theta(n^2)
      */
      var lowest = Infinity, j = 0, n = arr.length, a = n, temp;
      for (j = 0; j < n - 1; j += 1) {
        for (a = n; a >= j; a -= 1) {
          if (arr[a] < lowest) {
            lowest = arr[a];
